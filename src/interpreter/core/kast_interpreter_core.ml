...
let rec pattern_match : span:span -> place -> pattern -> (matched:bool * Scope.locals) =
  ...
  | P_Binding { bind_mode; binding } ->
    let ty, value =
      ...
      | ByRef { mut } ->
        ( Ty.inferred
            ~span
            (T_Ref { mut = IsMutable.new_inferred ~span mut; referenced = place.ty })
         ...
         := { by_symbol =
                SymbolMap.union
                  (fun symbol a _b ->
                     Error.error
                       span
                       "%a is matched multiple times in pattern"
                       Symbol.print
                       symbol;
                     Some a)
                  !matches.by_symbol
                  new_matches.by_symbol
            }
       in
       ...
       |> List.iter (fun (part : _ Types.tuple_part_of) ->
         ...
         | Field { label; label_span = _; field = field_pattern } ->
           ...
           (match Tuple.get_opt member tuple with
            | Some field ->
              pattern_match ~span field.place field_pattern |> update_matches
           ...
           (match
            ...
            | Some ty_tuple ->
              ...
              let placed =
                Place.init
                  ~mut:Inherit
                  (V_Tuple { tuple; ty = ty_tuple } |> Value.inferred ~span)
              in
              pattern_match ~span placed packed_pattern |> update_matches
            | None -> Error.error span "packed pattern type is not tuple?"));
       ~matched:!matched, !matches
     | value ->
       Error.error pattern.data.span "Expected tuple, got %a" Value.Shape.print value;
       ~matched:false, Scope.Locals.empty)
  | P_Variant { label = patern_label; label_span = _; value = value_pattern } ->
    (match place |> claim ~span |> Value.await_inferred with
     | V_Variant { label; data; ty = _ } ->
       if Label.equal label patern_label
       then (
...
and await_compiled ~span (f : Types.maybe_compiled_fn) : Types.compiled_fn option =
  if f.compiled |> Option.is_none
  then (
...
and call_untyped_fn
  ...
  with_return (fun { return } ->
    ...
    Log.trace (fun log ->
      log
        "evaled call (before sub) at %a = %a :: %a"
        Span.print
        span
        Value.print
        result
    ...
    Log.trace (fun log ->
      log
        "evaled call (after sub) at %a = %a :: %a"
        Span.print
        span
        Value.print
        result
  ...
  Kast_profiling.record
    ...
    (fun () ->
       ...
       fork (fun () ->
         ...
         Log.trace (fun log ->
           log
             "Inferred instantiation value at %a name = %a"
             Span.print
             span
             Value.print
             result);
         ...
         (match name with
          ...
          | Some name ->
            Log.trace (fun log ->
              log
                "Inferred instantiation at %a name = %a"
                Span.print
                span
                Kast_types.Print.print_name_shape
                name));
         match name with
         ...
         let actual_result =
           ...
           | V_Generic { name = _; fn; ty = _ } ->
             ...
             let save new_state =
               ...
               <- state.instantiated_generics.map |> Id.Map.add id generic_instantiations
             in
             let current_state =
               Kast_profiling.record
...
and assign : span:span -> state -> Expr.assignee -> parent_mut:bool -> place -> unit =
     ...
     | V_Tuple { ty = _; tuple } ->
       ...
       |> List.iter (fun (part : _ Types.tuple_part_of) ->
         ...
         | Field { label; label_span = _; field = assignee_field } ->
           ...
           in
           (match Tuple.get_opt member tuple with
            | Some field -> assign ~span state assignee_field ~parent_mut field.place
            | None -> Error.error span "no field %a" Tuple.Member.print member)
  ...
  | A_Let pattern ->
    ...
    if not matched then Error.error assignee.data.span "Failed to pattern match";
    state.scope |> Scope.add_locals new_bindings
  | A_Error -> ()

and error_place : ty -> place = fun ty -> Place.init_state ~mut:Inherit Uninitialized ty
...
and get_field ~span ~state ~(result_ty : ty) ~obj_mut obj member : evaled_place_expr =
  with_return (fun { return } ->
    let place : place =
      ...
      | V_Tuple { ty = _; tuple } ->
        (match Tuple.get_opt member tuple with
         ...
         | None ->
           Error.error span "field %a doesnt exist" Tuple.Member.print member;
      ...
      | V_Blocked blocked ->
        return
        <| RefBlocked
             { shape = BV_FieldRef { obj_ref = blocked; member }; ty = result_ty }
...
and eval_expr_ref : state -> expr -> Types.expr_ref -> value =
  ...
  | Place (~mut:place_mut, place) ->
    if mut && not place_mut
    then Error.error span "Constructing mutable ref to non-mut place";
...
and eval_expr_tuple : state -> expr -> Types.expr_tuple -> value =
  ...
  V_Tuple
    { ty
    ; tuple =
        !result (*        { place = Place.init value; span = label_span; ty_field }); *)
    }
  |> Value.inferred ~span
...
and eval_expr_if : state -> expr -> Types.expr_if -> value =
  ...
  | _ ->
    Error.error cond_expr.data.span "if cond must be bool, got %a" Value.print cond;
    V_Error |> Value.inferred ~span
    ...
    let result =
      ...
      |> List.find_map (fun (branch : Types.expr_match_branch) ->
        ...
        then
          Some
            (let inner_state =
               enter_scope ~span:branch.body.data.span ~recursive:false state
             in
...
and eval_expr_unwindable : state -> expr -> Types.expr_unwindable -> value =
  ...
  |> Scope.add_locals
       (pattern_match
          ~span:token_pattern.data.span
          (Place.init ~mut:Immutable token)
          token_pattern
        |> fun (~matched, locals) ->
        if not matched
        then Error.error token_pattern.data.span "Failed to pattern match unwind token";
        locals);
  try eval inner_state body with
  | Unwind { token; value } when token.id = id -> value
...
and eval_expr_unwind : state -> expr -> Types.expr_unwind -> value =
  ...
  with_return (fun { return } ->
    let token =
      eval state token_expr
      |> Value.expect_unwind_token
      |> Option.unwrap_or_else (fun () ->
...
and eval_expr_injectcontext : state -> expr -> Types.expr_inject_context -> value =
...
and eval_expr_targetdependent : state -> expr -> Types.expr_target_dependent -> value =
  ...
  with_return (fun { return } ->
    let chosen_branch =
      ...
      | None ->
        let computed =
          find_target_dependent_branch state target_dependent { name = "interpreter" }
          |> Option.unwrap_or_else (fun () ->
    ...
    (fun () ->
       try
         ...
         let result =
           ...
           | E_Newtype ty_expr ->
             let result_ty = Ty.new_not_inferred ~scope:state.result_scope ~span in
             fork (fun () ->
               let inner_ty = eval_ty state ty_expr in
               let name = OptionalName.new_inferred ~span (Some (current_name state)) in
               let newtype : Ty.Shape.t =
                 match inner_ty |> Ty.await_inferred with
                 | T_Tuple { name = _; tuple } -> T_Tuple { name; tuple }
                 | T_Variant { name = _; variants } -> T_Variant { name; variants }
                 | _ ->
  ...
  fun state target_dependent target ->
    ...
    |> List.find_map (fun (branch : Types.expr_target_dependent_branch) ->
      let scope_with_target =
        Scope.init ~span ~recursive:false ~parent:(Some target_dependent.captured)
      in
      scope_with_target
      |> Scope.add_local
           ~mut:false
...
and current_name : state -> Types.name_shape =
  fun state ->
  Log.trace (fun log ->
    log "Current name = %a" Kast_types.Print.print_name_shape state.current_name);
  state.current_name
    ...
    try
      ...
      let evaled_result =
        ...
        | TE_Expr expr ->
          let value = eval state expr in
          Log.trace (fun log ->
            log "evaled ty expr (expr) at %a = %a" Span.print span Value.print value);
          value
          |> Value.expect_ty
          |> Option.unwrap_or_else (fun () ->
            Error.error
        ...
        | TE_Tuple { guaranteed_anonymous = _; parts } ->
          ...
          Ty.inferred ~span
          <| T_Tuple { name = OptionalName.new_inferred ~span None; tuple = !tuple }
        | TE_Union { elements } ->
          let variants =
            elements
            |> List.map (fun ty_expr ->
              ...
              | T_Variant { name = _; variants } -> Row.await_inferred_to_list variants
              | _ ->
      ...
      in
      result |> Inference.Ty.expect_inferred_as ~span:expr.data.span evaled_result;
      Log.trace (fun log ->
        log "finished eval ty expr at %a = %a" Span.print span Ty.print evaled_result);
      Log.trace (fun log ->
        log
          "scope = %a, result scope = %a"
          Print.print_var_scope
...
and fork (f : unit -> unit) : unit =
  Kast_inference_base.fork (fun () ->
    ...
    | effect Scope.AwaitUpdate (name, scope), k ->
      let k = dont_leak_please k in
      scope.on_update <- (name, fun () -> k.continue true) :: scope.on_update
    | effect Inference.Var.AwaitUpdate var, k ->
      let k = dont_leak_please k in
