...
module Impl = struct
  module Interpreter = struct
    let instantiate
      : (?result_ty:ty -> span -> interpreter_state -> value -> value -> value) option ref
      =
      ref None
    ;;
    ...
    let claim_ref
      : (span:span -> state:interpreter_state -> result_ty:ty -> value -> value) option
          ref
      =
      ref None
    ;;
  ...
  let key ~(state : sub_state) =
    state.result_scope |> Option.map (fun (scope : interpreter_scope) -> scope.id)
  ;;
  ...
  let rec sub_place ~state (place : place) : place =
    ...
    | Uninitialized | MovedOut -> Place.init_state ~mut:place.mut place.state place.ty
  ...
  and sub_value_shape ~(state : sub_state) (original_value : value) (shape : value_shape)
    : value
    =
    let ctx = Effect.perform GetCtx in
    let span = ctx.span in
    Log.trace (fun log -> log "subbing value shape = %a" Value.Shape.print shape);
    let shaped shape = Value.inferred ~span shape in
    let result =
      match shape with
      | V_Unit | V_Bool _ | V_Int32 _ | V_Int64 _ | V_Float64 _ | V_Char _ | V_String _
  ...
  and sub_blocked ~original_value ~state (blocked : blocked_value) : value =
    ...
    | BV_Binding binding ->
      (match Scope.find_local_opt binding.name state.scope with
       | None ->
         if not (binding.scope |> VarScope.contains state.result_scope)
         then Error.error span "%a can't escape scope" Binding.print binding;
         original_value
       | Some local ->
         ...
         (match local.place.state with
          ...
          | Uninitialized | MovedOut ->
            Error.error span "Tried to sub with %a" Place.print_ref local.place;
            original_value))
    | BV_FieldRef { obj_ref; member } ->
      ...
      (match
         get_field ~result_ty:blocked.ty ~span ~state ~obj_mut:true obj_ref member
       with
       | RefBlocked ref -> V_Blocked ref |> Value.inferred ~span
       | Place (~mut, place) -> V_Ref { mut; place } |> Value.inferred ~span)
    | BV_ClaimRef ref ->
      let ref =
        sub_blocked ~original_value:(Value.inferred ~span (V_Blocked ref)) ~state ref
      in
  ...
  and sub_name_shape ~(state : sub_state) (shape : name_shape) : name_shape =
    ...
    | Instantiation { generic; arg } ->
      Log.trace (fun log ->
        log
          "subbing instantiation %a[%a] at %a"
          Value.print
          generic
          Value.print
          arg
  ...
  and sub_ty_variant ~(state : sub_state) ({ name; variants } as ty : ty_variant)
    : ty_variant
    =
    ...
    let variants =
      variants
      |> sub_row
           ~scope_of_value:VarScope.of_ty_variant_data
           ~unite_value:Inference_impl.unite_ty_variant_data
           ~state
           ~sub_value:(fun ~state ({ data } : ty_variant_data) : ty_variant_data ->
             { data =
                 ...
                 |> Option.map (fun ty ->
                   ...
                   Log.trace (fun log ->
                     log
                       "subbed variant data ty from %a to %a"
                       Ty.print
                       ty
                       Ty.print
                       result);
    ...
    Log.trace (fun log ->
      log
        "subbed ty_variant from %a (row scope = %a) to %a"
        Print.print_ty_variant
        ty
        Print.print_var_scope
        (Row.scope ty.variants)
        Print.print_ty_variant
        result);
    result
  ...
  and sub_ty_shape ~(state : sub_state) (original_ty : ty) (shape : ty_shape) : ty =
    let ctx = Effect.perform GetCtx in
    Log.trace (fun log ->
      log "subbing ty shape = %a at %a" Ty.Shape.print shape Span.print ctx.span);
    let shaped shape =
    ...
    let result =
      match shape with
      | T_Unit | T_Bool | T_Int32 | T_Int64 | T_Float64 | T_String | T_Char | T_Target
      ...
      | T_Blocked blocked ->
        let value =
          sub_blocked
            ~original_value:(V_Blocked blocked |> Value.inferred ~span:ctx.span)
            ~state
            blocked
        in
            ...
            |> List.map (fun (part : pattern tuple_part_of) : pattern tuple_part_of ->
              ...
              | Field { label; label_span; field } ->
                Field
                  { label
                  ; label_span
                  ; field = sub_pattern_and_inject_replacements ~state field
                  }
              | Unpack packed ->
                Unpack (sub_pattern_and_inject_replacements ~state packed))
        }
    | P_Variant { label; label_span; value } ->
      P_Variant
        { label
        ; label_span
        ; value = value |> Option.map (sub_pattern_and_inject_replacements ~state)
        }
    | P_Error -> P_Error
  ...
  and sub_row_shape
    ...
    | R_Cons { label; value; rest } ->
      ...
      <| R_Cons
           ...
           ; rest = sub_row ~scope_of_value ~unite_value ~sub_value ~state rest
           }
...
let sub_value ~span ~state value =
  ...
  Log.trace (fun log ->
    log
      "sub_value end at %a from %a to %a"
      Span.print
      span
      Value.print
      value
