...
module Uri = struct
  ...
  let print : formatter -> Uri.t -> unit =
    ...
    then fprintf fmt "%s" (Uri.path uri)
  ...
  let append_if_relative parent maybe_relative =
    ...
    | None ->
      ...
      in
      (match relative with
       | Some relative -> Uri.with_path parent (Uri.path parent ^ "/" ^ relative)
       | None -> maybe_relative)
    | _ -> maybe_relative
  ;;
    ...
    | None ->
      ...
      (match relative with
       | Some _relative ->
         ...
         let parent = String.sub file_path 0 (String.rindex file_path '/' + 1) in
