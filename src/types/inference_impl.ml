...
module VarScopeImpl = struct
  ...
  let rec unite (a : t) (b : t) : t =
    ...
    | Some a, Some b ->
      ...
      if diff < 0
      then unite (Some a) b.parent
      else if diff > 0
      then unite a.parent (Some b)
      else if Id.equal a.id b.id
      then Some a
  ...
  let rec contains (child : t) (ansestor : t) : bool =
    ...
    | Some child, Some ansestor ->
      ...
      then contains child.parent (Some ansestor)
      else if Id.equal child.id ansestor.id
      then true
      else false
  ;;
...
module VarScope = struct
  ...
  and of_is_mutable : is_mutable -> var_scope = fun { var } -> Inference.Var.scope var
  ...
  and of_value_variant : value_variant -> var_scope =
    fun { label = _; data; ty } -> deepest (of_option of_place data) (of_ty_variant ty)
  ...
  and of_value_target : value_target -> var_scope = fun { name : string = _ } -> root ()
  and of_value_context_ty : value_context_ty -> var_scope = fun { id = _; ty } -> of_ty ty
...
let unite_with_ctx (f : 'a Inference.unite) ~span a b = with_ctx (fun () -> f ~span a b)
...
module Impl = struct
  ...
  and unite_blocked_value_shape : blocked_value_shape Inference.unite =
    ...
    let fail () : blocked_value_shape =
      error
        span
        "blocked_value_shape %a != %a"
        print_blocked_value_shape
        a
        print_blocked_value_shape
        b;
      a
    ...
    | ( BV_FieldRef { obj_ref = obj_a; member = member_a }
      ...
      BV_FieldRef { obj_ref = unite_blocked_value ~span obj_a obj_b; member = member_a }
    ...
    let result : ty_generic =
      ...
      let sub_with (bindings : (binding * binding) list) ty =
        let target_scope =
          bindings
          |> List.map snd
          |> List.fold_left (fun acc binding -> VarScope.deepest acc binding.scope) None
        in
        Log.trace (fun log -> log "target_scope = %a" Print.print_var_scope target_scope);
          ...
          let locals : Types.interpreter_locals =
            { by_symbol =
                ...
                |> List.map (fun (a, b) : (symbol * interpreter_local) ->
                  let binding =
                    V_Blocked { shape = BV_Binding b; ty = b.ty } |> inferred_value ~span
                  in
        ...
        Log.trace (fun log -> log "subbed %a into %a" print_ty ty print_ty result);
        result
      in
      ...
      Log.trace (fun log ->
        log
          "unite_generic_ty result_scope: %a and %a"
          print_var_scope
          (VarScope.of_ty result_a)
          print_var_scope
          (VarScope.of_ty result_b));
      let result_a_subbed = result_a |> sub_with bindings_ab in
      Log.trace (fun log ->
        log "unite_generic_ty a->b %a and %a" print_ty result_a_subbed print_ty result_b);
  ...
  and unite_pattern : span:span -> pattern -> pattern -> (binding * binding) list =
    fun ~span a b ->
    let fail () =
      let print_pattern = print_pattern ~options:{ spans = false; types = false } in
  ...
  and unite_ty_ref : ty_ref Inference.unite =
    fun ~span { mut = mut_a; referenced = ref_a } { mut = mut_b; referenced = ref_b } ->
    { mut = unite_is_mutable ~span mut_a mut_b; referenced = unite_ty ~span ref_a ref_b }
  ...
  and unite_ty_variant : ty_variant Inference.unite =
    ...
    let variants =
      Log.trace (fun log ->
        log
          "%a :: Unifying rows %a and %a"
          Id.print
          unite_id
          print_var_scope
          (Row.scope variants_a)
          print_var_scope
          (Row.scope variants_b));
      Log.trace (fun log ->
        log
          "common scope = %a"
          print_var_scope
      ...
      let row =
        Row.unite
          ...
          ~span
          unite_ty_variant_data
          variants_a
          variants_b
      in
      Log.trace (fun log ->
        log "%a :: United rows into %a" Id.print unite_id print_var_scope (Row.scope row));
      row
    in
  ...
  and unite_ty_tuple : ty_tuple Inference.unite =
    fun ~span
      ({ name = name_a; tuple = a } as tuple_a)
    ...
    with
    | Invalid_argument _ ->
      error span "ty_tuple %a != %a" print_ty_tuple tuple_a print_ty_tuple tuple_b;
      tuple_a

  and unite_ty_tuple_field =
    fun ~span
      (a : Types.ty_tuple_field)
  ...
  and unite_optional_name : optional_name Inference.unite =
    fun ~span { var = a } { var = b } ->
    { var =
        Inference.Var.unite (unite_option unite_name_shape) VarScopeImpl.unite ~span a b
    }
  ...
  and unite_name_instantiation : name_instantiation Inference.unite =
    fun ~span { generic = generic_a; arg = arg_a } { generic = generic_b; arg = arg_b } ->
    { generic =
        error_context
          ...
          (fun fmt ->
             fprintf
               fmt
               "different generics are instantiated: %a != %a"
               print_value
               generic_a
               print_value
               generic_b)
  ...
  and unite_name_shape : name_shape Inference.unite =
    ...
    error_context
      ...
      (fun fmt -> fprintf fmt "name_shape %a != %a" print_name_shape a print_name_shape b)
           ...
           ; tuple =
               ...
               |> Tuple.map (fun (field : ty_tuple_field) : value_tuple_field ->
                 { place =
                     ...
                     <| new_not_inferred_value_of_ty ~scope ~span field.ty
                 ; span
                 ; ty_field = field
                 })
           })
    ...
    fun shape ->
      ...
      | V_Ref { mut; place } ->
        inferred_ty
          ~span
          (T_Ref
             { mut = { var = Inference.Var.new_inferred VarScope.of_bool ~span mut }
             ; referenced = place.ty
             })
      ...
      | V_NativeFn { id = _; ty; name = _; impl = _ } -> inferred_ty ~span <| T_Fn ty
