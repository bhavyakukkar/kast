...
module Impl = struct
  ...
  and print_blocked_value_shape : formatter -> blocked_value_shape -> unit =
    ...
    | BV_FieldRef { obj_ref; member } ->
      fprintf fmt "(&%a.%a)" print_blocked_value obj_ref Tuple.Member.print member
  ...
  and print_ty_tuple : always_print_shape:bool -> formatter -> ty_tuple -> unit =
    ...
    print_optionally_named ~always_print_shape fmt name (fun fmt ->
      fprintf
        ...
        "%a"
        (Tuple.print
           ~options:{ Tuple.default_print_options with named_field_middle = " :: " }
  ...
  and print_var : 'a. (formatter -> 'a -> unit) -> formatter -> 'a var -> unit =
    ...
    if cache |> RecurseCache.depth id > 0
    then (
      is_recursive |> RecurseCache.enter id;
      fprintf fmt "<recursive %a>" Id.print id)
    else (
      cache |> RecurseCache.enter id;
      if is_recursive |> RecurseCache.is_visited id then fprintf fmt "<id=%a>" Id.print id;
  ...
  and print_expr_shape : options:options -> formatter -> expr_shape -> unit =
    ...
    | E_Assign { assignee; value } ->
      fprintf
        ...
        (print_place_expr ~options)
        value
    | E_Ty expr -> fprintf fmt "@{<magenta>type@} %a" (print_ty_expr ~options) expr
    | E_Newtype e ->
      fprintf
        fmt
        "@{<magenta>newtype@} %a"
    ...
    | E_If { cond; then_case; else_case } ->
      fprintf
        ...
        "@{<magenta>if@} %a"
        (Tuple.print (print_expr ~options))
        (Tuple.make [] [ "cond", cond; "then_case", then_case; "else_case", else_case ])
    | E_And { lhs; rhs } ->
      fprintf
        fmt
    ...
    | E_TargetDependent { branches; captured = _; interpreter_branch = _ } ->
      ...
      |> List.iter (fun branch ->
        fprintf fmt "@;<0 2>@[<v>%a@]" (print_target_dependent_branch ~options) branch);
      fprintf fmt " )"
  ...
  and print_ir_tuple_part
    : 'a. char -> (formatter -> 'a -> unit) -> formatter -> 'a tuple_part_of -> unit
    =
  ...
  and print_ir_tuple_field
    : 'a. char -> (formatter -> 'a -> unit) -> formatter -> 'a tuple_field_of -> unit
    =
  ...
  and print_expr_match_branch : options:options -> formatter -> expr_match_branch -> unit =
    fun ~options fmt { pattern; body } ->
    fprintf
      fmt
      "(@;<0 2>@[<v>pattern = %a,@]@;<0 2>@[<v>body = %a@]@ )"
      (print_pattern ~options)
      pattern
      (print_expr ~options)
      body
  ...
  and print_assignee_expr_shape
    ...
    | A_Place place ->
      fprintf
        fmt
        "@{<magenta>place@} (@;<0 2>place = %a@ )"
        (print_place_expr ~options)
        place
    | A_Tuple tuple -> print_ir_tuple '=' (print_assignee_expr ~options) fmt tuple
    | A_Let pattern ->
      fprintf
        fmt
        "@{<magenta>let@} (@;<0 2>pattern = %a@ )"
        (print_pattern ~options)
        pattern
    | A_Error -> fprintf fmt "@{<red><error>@}"

  and print_assignee_expr : options:options -> formatter -> assignee_expr -> unit =
    fun ~options fmt { shape; data } ->
  ...
  and print_ty_expr_shape : options:options -> formatter -> ty_expr_shape -> unit =
    fun ~options fmt -> function
    | TE_Unit -> fprintf fmt "()"
  ...
  (* PATTERN *)
  and print_pattern_shape : options:options -> formatter -> pattern_shape -> unit =
    fun ~options fmt -> function
    ...
    | P_Variant { label; label_span = _; value } ->
      fprintf fmt ":%a %a" Label.print label (Option.print (print_pattern ~options)) value
  ...
  and print_name_shape_part : formatter -> name_part -> unit =
    fun fmt -> function
    | Uri uri ->
      let name = Uri.path uri |> Filename.basename |> Filename.remove_extension in
      fprintf fmt "%s" name
...
let print_place_expr_shape =
  with_cache (Impl.print_place_expr_shape ~options:{ spans = true; types = false })
;;

let print_expr_shape ~options = with_cache (Impl.print_expr_shape ~options)
let print_assignee_expr ~options = with_cache (Impl.print_assignee_expr ~options)
...
let print_ty_expr_shape ~options = with_cache (Impl.print_ty_expr_shape ~options)
